...
<section class="hype-w-85x100 mx-auto py-5">
    ...
    <div class="alert alert-danger">
        c'è un errore nel nome inserito.
    </div>
    @enderror
                ...
                <tr>
                    <form action="{{route('admin.slots.update', $slot->id)}}" method="POST" novalidate>
                        ...
                        <td class="hype-w-15x100">
                            <p class="td paragraph @error('start_time') err-animation @enderror {{$slot->id}}">{{$slot->start_time}} </p>
                     ...
                     <td class="hype-w-40x100">
                        <div class="d-flex justify-content-center">
                            ...
                            <a href="{{ route('admin.slots.edit', $slot->id) }}" class="edit-button table-icon m-1 icon-{{$slot->id}}"  id="{{$slot->id}}">
                                <div class="icon-container">
                                    <i
                                        class=" fa-solid fa-pen-to-square fs-3 text-active-tertiary hype-text-shadow hype-hover-size"></i>
                                </div>
                            </a>
                            <button  class="mine-custom-btn edit-a table-icon m-1 icon-{{$slot->id}} d-none" type="submit"  id="{{$slot->id}}">
                                <div class="icon-container">
                                    <i
                            ...
                            <form id="delete-form" action="{{ route('admin.slots.destroy', $slot->id) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
