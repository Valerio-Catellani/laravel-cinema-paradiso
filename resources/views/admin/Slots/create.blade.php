@section('title', 'Aggiungi una Fascia Oraria')
...
@section('content')
    <section class="container py-5">
        ...
        <div class="container rounded-2 hype-shadow-white p-5 container-table">
            <h1 class="text-center hype-text-shadow text-white fw-bolder">Aggiungi una Fascia Oraria</h1>

            <form id="comic-form" action="{{ route('admin.slots.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-3 @error('name') err-animation @enderror">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid err-animation @enderror"
                        id="name" name="name" value="{{ old('name') }}" required maxlength="200" minlength="3">
                    @error('name')
